...
export default function SlotDuel({ player, trainer, flutuante }: SlotDuelProps) {
  return (
    <div className="relative flex items-end justify-between gap-4 px-2 py-6">
      <Lado key={`p-${player.id}`} id={player.id} nome={player.nome} hp={player.hp} hpMax={player.hpMax} alinhamento="left" />
      <div className="text-2xl font-black text-white/40 self-center">VS</div>
      <Lado key={`t-${trainer.id}`} id={trainer.id} nome={trainer.nome} hp={trainer.hp} hpMax={trainer.hpMax} alinhamento="right" />
      {flutuante && (
        <div
